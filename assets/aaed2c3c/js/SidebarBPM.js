...
{
        //addStencilPalette(id, title, stencilFile, style, ignore, onInit, scale)
	this.addStencilPalette('bpmn', 'Bussiness Process Model', dir + '/bpmn.xml',
		';fillColor=#ffffff;strokeColor=#000000;perimeter=ellipsePerimeter;',
		['Cancel', 'Error', 'Link', 'Message', 'Compensation', 'Multiple', 'Rule', 'Timer'],
		function(content)
		{
			content.appendChild(this.createVertexTemplate('swimlane;horizontal=1;', 600, 180, 'Divisi','divisi'));
                        content.appendChild(this.createVertexTemplate('swimlane;horizontal=1;', 300, 160, 'Entitas','entitas'));
                        
			var classCell = new mxCell('Process', new mxGeometry(0, 0, 100, 40),
		    	'rounded=1');
			classCell.vertex = true;
                        classCell.name = 'proses';                       
                        
                        
			content.appendChild(this.createVertexTemplateFromCells([classCell], 100, 40));	
                        
                        content.appendChild(this.createVertexTemplate('rhombus;horizontal=1;', 140, 60, 'Kondisi','kondisi'));
 ...
 * Creates a drop handler for inserting the given cells.
 */
Sidebar.prototype.createVertexTemplate = function(style, width, height, value, name)
{
...
/**
 * Add Properties to the sidebar.
 */
Sidebar.prototype.addPropertiesForm = function(expand)
{
