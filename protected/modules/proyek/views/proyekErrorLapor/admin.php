<?php
$this->breadcrumbs=array(
	'Proyek Error Lapors'=>array('index'),
	'Manage',
);

$this->menu=array(
	array('label'=>'List ProyekErrorLapor','url'=>array('index')),
	array('label'=>'Create ProyekErrorLapor','url'=>array('create')),
);

Yii::app()->clientScript->registerScript('search', "
$('.search-button').click(function(){
	$('.search-form').toggle();
	return false;
});
$('.search-form form').submit(function(){
	$.fn.yiiGridView.update('proyek-error-lapor-grid', {
		data: $(this).serialize()
	});
	return false;
});
");
?>

<h1>Manage Proyek Error Lapors</h1>

<p>
You may optionally enter a comparison operator (<b>&lt;</b>, <b>&lt;=</b>, <b>&gt;</b>, <b>&gt;=</b>, <b>&lt;&gt;</b>
or <b>=</b>) at the beginning of each of your search values to specify how the comparison should be done.
</p>

<?php echo CHtml::link('Advanced Search','#',array('class'=>'search-button btn')); ?>
<div class="search-form" style="display:none">
<?php $this->renderPartial('_search',array(
	'model'=>$model,
)); ?>
</div><!-- search-form -->

<?php $this->widget('bootstrap.widgets.TbGridView',array(
	'id'=>'proyek-error-lapor-grid',
	'dataProvider'=>$model->search(),
	'filter'=>$model,
	'columns'=>array(
		'proyek_error_lapor_id',
		'proyek_error_lapor_judul',
		'proyek_error_lapor_tanggal',		
                array('name'=>'proyek_error_lapor_konten','header'=>'Konten','value'=>'$data->proyek_error_lapor_konten','type'=>'raw'),
		'proyek_error_lapor_status',		
                array('name'=>'anggota_id','header'=>'Anggota','value'=>'$data->anggota->anggota_realname'),
		/*
		'proyek_sub_id',
		*/
		array(
			'class'=>'bootstrap.widgets.TbButtonColumn',
		),
	),
)); ?>
